/************************************************************
 *  ESP32-C3 + Nokia 5110 (PCD8544) — Cellular Automaton
 *
 *  IMPORTANT (Arduino IDE):
 *  The Arduino IDE auto-generates function prototypes.
 *  If a prototype uses a type declared later (e.g., RuleId),
 *  you can get: "RuleId was not declared in this scope".
 *
 *  => Put enum RuleId AT THE VERY TOP (before #include),
 *     and avoid the name "ruleName" -> use "ruleNameStr".
 *
 *  “Life-like” rules:
 *   - Conway    : B3/S23 (classic)
 *   - HighLife  : B36/S23 (replicators)
 *   - Seeds     : B2/S0 (explosive, nothing survives)
 *   - DayNight  : B3678/S34678 (organic)
 *   - LWD       : B3/S012345678 (Life Without Death)
 *   - 2x2       : B36/S125 (blocky patterns)
 *
 *  RUNNING controls:
 *   - T short : (MANUAL) toggle cell under cursor
 *   - T long  : show/hide overlay (A/M + generations)
 *   - P short : (AUTO) play/pause | (MANUAL) step 1 generation
 *   - P long  : go to SETUP
 *   - M short : MANUAL <-> AUTO
 *   - M long  : RESET (clear world + gen=0)
 *
 *  SETUP controls:
 *   - POT_X : Delay 15..5000 ms
 *   - POT_Y : Inject 0..100 (on your wiring: right = low ADC => inverted)
 *   - M short : MANUAL/AUTO
 *   - P short : change rule (cycle)
 *   - T short : START
 ************************************************************/

// ✅ Put the enum here (before #include) to avoid Arduino auto-prototype issues
enum RuleId {
  RULE_CONWAY,
  RULE_HIGHLIFE,
  RULE_SEEDS,
...
// You said: POT_X min -> bottom; POT_Y min -> right
static const bool SWAP_AXES = true;  // potY->X, potX->Y
static const bool INVERT_X  = true;  // low ADC -> right
static const bool INVERT_Y  = true;  // low ADC -> bottom
...
static const bool WRAP_EDGES = false; // true = torus wrap, false = edges are empty

static const uint16_t INJECT_EVERY = 30; // inject every N generations (AUTO + running)
...
// =====================
// Rules: selection + name
// =====================

// ✅ Current rule (changeable in SETUP via P short)
RuleId currentRule = RULE_CONWAY;

// Rule name (displayed instead of "SETUP")
...
// Rule: returns the next state based on currentRule
inline bool nextAliveForRule(bool alive, int n) {
  switch (currentRule) {
    ...
    case RULE_SEEDS:    // B2/S0 (no survival)
...
// Overlay: if false => hide EVERYTHING (letter + number)
bool showGenOverlay = true;

// Format generation counter as k/M/G (readable on 84x48)
...
void drawWorld(bool overlay) {
  ...
  // Cursor in MANUAL (3x3 rectangle)
  ...
  // Overlay: if hidden => draw nothing at all
...
void loop() {
  ...
  // P short:
  // - in SETUP: change rule
  // - in RUNNING: play/pause (AUTO) or step (MANUAL)
